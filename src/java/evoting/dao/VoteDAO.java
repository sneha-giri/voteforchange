...
public class VoteDAO {
    ...
    static {
        try {
            ...
            ps2 = DBConnection.getConnection().prepareStatement("select candidate_id,username,party,symbol from candidate,user_details where candidate.userid=user_details.adhar_no and candidate.candidate_id=?");
            ps3 = DBConnection.getConnection().prepareStatement("insert into voting values(?,?)");
            ps4 = DBConnection.getConnection().prepareStatement("select candidate_id,count(*) as vote_obt from voting group by candidate_id order by votes_obt desc");
    ...
    public static CandidateInfo getVote(String candidateid) throws Exception {
        ...
        if (rs.next()) {
            ...
            cd.setCandidateName(rs.getString(3));
